...
⚠️ **作废声明**：本节早先写过一版"轨迹归因已完成、核心结论=30 点差距主体是 Agent 行为"，并附了一串具体数字（"全 369 分桶 WIN 151/INF_miss 17/ZERO_feasible 178""178 个里空轨迹=0、中途停 7→环境崩溃可忽略""120 个模型主动 answer 自认完成""infeasible 12 全满分/17 全 0 零错配""libreoffice 有 Ctrl+S 0.637/无 0.365、40 个没保存"）。**这些聚合数字与结论都已作废，不可信**——它们没有真正跑遍并逐个核对全部样本就写下，且把 result.txt 里的 0 分**默认当成"模型失败"**来分桶，而"这些 0 里有多少其实是网络/环境/权限故障"恰恰是当前还没查清、最该先排除的问题（下一节 §"深挖更新"也已指出原始逐任务数据被多-episode 污染、单样本噪声极大）。

**真实现状：诊断仍在进行，当前工作 = 先排除网络 / 环境 / 权限等影响因素，再谈模型能力。** 在逐任务用 report.html 把"环境/权限失败"与"模型失败"分清（即 Phase 2 的归桶）之前，**不下"差距主因是 Agent 行为"这类绝对结论**。只看过的少数个例（如 libreoffice calc `1273e544` 疑似漏保存、chrome `e1e75309` 评测器假阴性）只能当线索，不能外推到全体。

**待排除/待答**：① 网络（VM geo/代理是否每任务真生效、setup 下载与评测期 getter 是否偶发失败被记成 0）；② 评测器慢代理假阴性（见记忆 `osworld-evaluator-networkidle`，已修一批，可能还有隐藏）；③ 权限类任务是否因环境配置而非模型失败；④ 需登录的 Google Drive 任务；⑤ harness/system prompt/工具集是否与官方一致（官方 4B 也 75.8%）。→ 按 Phase 0/1/2 推进。⚠️ 在确认官方做法前，**不要**私自加"自动存盘 hack"或"记得保存"提示注入——那是不忠实的改动。
...
**结论修正**：① 个例里**观察到**一些疑似模型失败模式（不稳的精细 grounding（单字符/精确区域选择）、精度（公式/格式）、不自检、漏保存）——但这只是少数个例线索，**占多大比例、是否就是差距主因，当前数据都答不了**，且网络/环境/权限故障被记成 0 的部分尚未排除；② "漏保存占多大""v2 好不好"同样答不了——基线被污染、单样本噪声盖过效应。**要得到可信结论必须：(a) 先把环境/权限/网络失败的 0 从"模型失败"里分出来（Phase 2 归桶）；(b) 用修好的 recorder 重跑得干净基线；(c) 多采样（每任务跑 k 次取均值）压噪声后再比 prompt，且子集要随机不按分数选。**
