...
- **轨迹诊断结论**：30 点差距主体是 **Agent 行为**，不是环境故障——空轨迹假 0 极少；最大单一可修因是 **libreoffice 漏保存**（改完没存盘 → 评测器读磁盘旧文件 → 0）。下一步最高价值是与官方 harness / system prompt 对齐（官方未公布 prompt，正在做 v1/v2 提示词 A/B）。
